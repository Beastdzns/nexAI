...
import { EchelonClient } from '@nexai/echelon-sdk';

import { getCoinList } from '../constants/aptos-coins';
      ...
      for (let i = 0; i < markets.length; i += chunkSize) {
        ...
        const results = await Promise.all(
          chunks.map(async (market: any) => {
            ...
            const coin = getCoinList('mainnet').find(
