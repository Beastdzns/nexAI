...
import { AptosNetworkName, getCoinList } from '../constants/aptos-coins';

export const searchCoinTool = tool(
  async ({ query, network }) => {
    const list = getCoinList(network as AptosNetworkName | undefined);
  ...
  },
  {
    ...
    schema: z.object({
      ...
      network: z
        ...
        .optional()
        .describe('Aptos network to search (default mainnet).'),
    }),
  },
);

export const getCoinTool = tool(
  async ({ coin_type, network }) => {
  ...
  },
  {
    ...
    schema: z.object({
      ...
      network: z
        .enum(['mainnet', 'testnet'])
        .optional()
        .describe('Aptos network to use (default mainnet).'),
    }),
  },
);
